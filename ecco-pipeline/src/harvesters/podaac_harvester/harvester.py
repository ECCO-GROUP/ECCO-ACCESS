...
def clean_solr(config, solr_host, grids_to_use, solr_collection_name):
    ...
    if not grids_to_use:
        fq = ['type_s:grid']
        docs = solr_query(solr_host, fq, solr_collection_name)
    ...
    dataset_metadata = solr_query(solr_host, fq, solr_collection_name)
    ...
    if grids:
        solr_update(solr_host, update_body, solr_collection_name)
...
def solr_update(solr_host, update_body, solr_collection_name, r=False):
    """
...
def podaac_harvester(config, output_path, LOG_TIME, s3=None, on_aws=False, solr_info='', grids_to_use=[]):
    ...
    existing_descendants_docs = solr_query(solr_host, fq, solr_collection_name)
    ...
    if entries_for_solr:
        ...
        r = solr_update(solr_host, entries_for_solr,
    ...
    failed_harvesting = solr_query(solr_host, fq, solr_collection_name)

    # Query for Solr successful harvest documents
    fq = ['type_s:granule',
          f'dataset_s:{dataset_name}', f'harvest_success_b:true']
    successful_harvesting = solr_query(solr_host, fq, solr_collection_name)
    ...
    dataset_query = solr_query(solr_host, fq, solr_collection_name)
    ...
    if not update:
        ...
        r = solr_update(solr_host, [ds_meta], solr_collection_name, r=True)
        ...
        field_query = solr_query(solr_host, fq, solr_collection_name)
        ...
        if body:
            ...
            r = solr_update(solr_host, body, solr_collection_name, r=True)
    ...
    # if dataset entry exists, update download time, converage start date, coverage end date
        ...
        r = solr_update(solr_host, [update_doc], solr_collection_name, r=True)

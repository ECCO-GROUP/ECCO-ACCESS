...
def find_podaac_metadata(podaac_dataset_table, filename, debug=False):
    ...
    if "_snap_" in filename:
        ...
        raise Exception("Error: filename may not conform to ECCO V4r5a convention.")

    if debug:
        print('split filename into ', head, tail)

    tail = tail.split("_ECCO_V4r5a_")[1]
...
def generate_netcdfs(output_freq_code, job_id:int, num_jobs:int, \
    ...
    dataset_description_tail_latlon = ' interpolated to a regular 0.5-degree grid from the ECCO Version 4 release 5a (V4r5a) ocean and sea-ice state estimate.'
...
def create_parser():
    ...
    parser.add_argument('--grouping_to_process', required=True, type=int,\
                        help='which dataset grouping to process, there are 20 in V4r5a')
...
#        output_dir_base=Path('/home/ifenty/tmp/v4r5a_nc_output_20210603')
    ...
    #ecco_grid_filename = 'ECCO_V4r5a_llc90_grid_geometry.nc'
    #ecco_grid_dir     = Path('/nobackupp2/ifenty/grids/grid_ECCOV4r5a')
    #ecco_grid_dir_mds = Path('/nobackupp2/ifenty/grids/grid_ECCOV4r5a/')

    #podaac_dir = Path('/home5/ifenty/git_repos_others/ecco-data-pub/metadata')
    #metadata_json_dir = Path('/home5/ifenty/git_repos_others/ECCO-GROUP/ECCO-ACCESS/metadata/ECCOV4r5a_metadata_json')
    #diags_root = Path('/nobackupp11/owang/runs/V4r5a/')

...
transformation_template_path = Path(f'{Path(__file__).parents[1]}/preprocessing/grid_transformation')
aggregation_template_path = Path(f'{Path(__file__).parents[1]}/preprocessing/aggregation_by_year')
podaac_template_path = Path(f'{Path(__file__).parents[1]}/harvesters/podaac_harvester')
osisaf_template_path = Path(f'{Path(__file__).parents[1]}/harvesters/osisaf_ftp_harvester')
nsidc_template_path = Path(f'{Path(__file__).parents[1]}/harvesters/nsidc_ftp_harvester')
...
if add_new_directories:
    ...
    if harvest_type == 'podaac':
        new_harvester_path = Path(f'{path_to_harvesters}/podaac_harvester/{ds_name}')
        template_path_to_use = podaac_template_path
    elif harvest_type == 'nsidc':
        new_harvester_path = Path(f'{path_to_harvesters}/nsidc_ftp_harvester/{ds_name}')
        template_path_to_use = nsidc_template_path
    elif harvest_type == 'osisaf':
        new_harvester_path = Path(f'{path_to_harvesters}/osisaf_ftp_harvester/{ds_name}')
    ...
    new_transformation_path = Path(f'{path_to_preprocessing}/{ds_name}/grid_transformation')
    if not os.path.exists(new_transformation_path):
        os.makedirs(new_transformation_path)
    for template_file in os.listdir(transformation_template_path):
        if '.DS' not in template_file and '__pycache__' != template_file:
            source_file = Path(f'{transformation_template_path}/{template_file}')
            destination_file = Path(f'{new_transformation_path}/{template_file}')
            copyfile(source_file, destination_file)

    #aggregation ---------------------------------------------------------------------------------------------------------------------
    #create new directories and add aggregation template files
    new_aggregation_path = Path(f'{path_to_preprocessing}/{ds_name}/aggregation_by_year')
